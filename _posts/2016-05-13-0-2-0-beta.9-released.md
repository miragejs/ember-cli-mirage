...
While this is pushing back the release, I think it's important to get right. I also feel like I have a better grasp of something important, namely that Mirage as an HTTP faking layer should emulate concepts that come from HTTP, rather than specific server abstractions. It turns out that single-owner foreign keys is not one of those concepts, and therefore this abstraction does not belong in Mirage.
